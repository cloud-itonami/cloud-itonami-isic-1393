(ns carpetops.render-html
  "Build-time HTML renderer for `docs/samples/operator-console.html`.

  Closes flagship checklist item 2 (com-junkawasaki/root ADR-2607189300):
  this repo previously had NO demo page and no generator at all. This
  namespace drives the REAL actor stack (`carpetops.operation` ->
  `carpetops.governor` -> `carpetops.store`) through a scenario adapted
  from this repo's own `carpetops.sim` demo driver (`clojure -M:run`,
  confirmed against `carpetops.store/sample-batches`/`sample-equipment`
  seeded ids before this file was written), trimmed to a representative
...
(defn run-demo!
  ...
  `carpetops.store/sample-batches`/`sample-equipment`:

  batch-001 (first-quality cut-pile-tufted-carpet, verified+registered,
  20000.0 m² produced, 5000.0 m² already shipped): a
  ...
  op in phase 3's `:auto` set -- `carpetops.phase`).

  mnt-1 on equip-001 (verified+registered tufting-machine):
  ...
  :coordination/safety-concern`, in `carpetops.governor/high-stakes`
  -- ALWAYS escalates regardless of confidence, approved by a human plant
  supervisor.

  ship-1 on batch-001 (5000.0 m² claim; 5000.0 already shipped + 5000.0
  claimed = 10000.0, within batch-001's own recorded 20000.0 m²
  ...
  human (a human approver cannot override a HARD violation):
    - mnt-2 on equip-002 (seeded `:verified? false :registered? false`
      weaving-loom): `:schedule-maintenance` HARD-holds on
    ...
    - ship-2 on batch-003 (seeded `:verified? false :registered? false`
      substandard berber-loop-carpet-tile batch): `:coordinate-shipment`
      HARD-holds on `:batch-not-verified` -- same independent-verification
      discipline, applied to the batch side.
    - ship-3 on batch-002 (own recorded 6000.0 m² produced, 5700.0 m²
      already shipped; a further 1000.0 m² claim would total 6700.0):
      `:coordinate-shipment` HARD-holds on `:shipment-volume-exceeded` --
      the governor independently recomputes shipped-to-date + claimed
      against the batch's own recorded production area, never trusts the
      claim.
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (exec! actor "b1-log" {:op :log-production-batch :effect :propose :subject "batch-001"
                            :patch {:quality-grade :first-quality :last-assessed "2026-07-14"}})

    ;; mnt-1: maintenance scheduling against equip-001 (verified+registered
    ;; tufting-machine) -- ALWAYS escalates, approved by a human plant
    ;; supervisor.
    ...
    (exec! actor "c1-flag" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                             :value {:equipment-id "equip-001" :concern-type :equipment-safety
                                     :severity :moderate
                                     :description "タフティング機の針板ガード固定に緩みを確認"}})
    ...
    ;; ship-3: shipment coordination against batch-002 (1000.0 m² claim
    ;; would push 5700.0 already-shipped past its own recorded 6000.0 m²
    ;; produced) -> HARD hold :shipment-volume-exceeded,
...
(defn- batch-row [ledger {:keys [id product quality-grade area-square-meters
                                  pile-density-tufts-per-sqm tuft-bind-strength-lbf
  ...
  (format "        <tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>"
          (esc id) (esc (or product "—"))
          (esc (name (or quality-grade :n-a)))
          (esc area-square-meters)
          (esc pile-density-tufts-per-sqm)
          (esc tuft-bind-strength-lbf)
          ...
          (esc shipped-area-square-meters)
...
(def ^:private action-gate-rows
  ...
  ;; (`carpetops.governor`/`carpetops.phase`) -- documentation of
  ...
  ["        <tr><td><code>:log-production-batch</code></td><td><span class=\"ok\">phase-3 auto-commit when clean, no physical/financial risk yet -- the ONLY auto-eligible op in this domain</span></td></tr>"
   ...
   "        <tr><td><code>:flag-safety-concern</code></td><td><span class=\"warn\">ALWAYS human approval regardless of confidence &middot; equipment-safety/quality-defect concerns are never gated on the referenced equipment being verified</span></td></tr>"
   "        <tr><td><code>:coordinate-shipment</code></td><td><span class=\"warn\">ALWAYS human approval &middot; batch verified?/registered? independently re-derived &middot; claimed area independently recomputed against the batch's own logged shipped-to-date + production area (m²), never trusted from the proposal</span></td></tr>"])
...
(defn render
  ...
  (let [ledger (vec (store/ledger db))
    ...
    (str
     "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-1393 &middot; manufacture of carpets and rugs</title><style>"
   (jp-go-dds.skin/dds+skin)
   "</style></head><body>\n"
     "<header class=\"bar\">\n"
     "  <h1>Manufacture of carpets and rugs (ISIC 1393) — Operator Console</h1>\n"
     ...
     "    <p class=\"muted\">Demo snapshot — build-time-generated from <code>carpetops.store</code> via <code>carpetops.render-html</code> (<code>clojure -M:dev:render-html</code>), regenerated nightly.</p>\n"
     "    <table>\n"
     "      <thead><tr><th>Batch</th><th>Product</th><th>Quality grade</th><th>Area (m²)</th><th>Pile density (tufts/m²)</th><th>Tuft-bind (lbf)</th><th>Defect rate (%)</th><th>Verified</th><th>Registered</th><th>Shipped area (m²)</th><th>Last op status</th></tr></thead>\n"
     "      <tbody>\n"
     batch-rows "\n"
     "      </tbody>\n"
     "    </table>\n"
     ...
     "    <p class=\"muted\">Append-only equipment-safety/quality-defect concern log — always human-reviewed, never gated on equipment verification status.</p>\n"
     "    <table>\n"
