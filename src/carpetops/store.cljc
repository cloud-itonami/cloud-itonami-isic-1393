(ns carpetops.store
  "SSoT for the carpet/rug-plant operations coordination actor, behind
  ...
  external carpet capability library, no jurisdiction-scoped Datomic
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             tufting/weaving/backing output:
                             quality-grade/area/pile-density/tuft-
                             bind-strength/defect-rate record (tufted
                             or woven carpet/rug). `:verified?` marks
                             whether the batch's own claims have
                             actually been QC-inspected --
                             specifically, whether the batch's own
                             tuft-bind-strength-test result has
                             actually been measured (never inferred
                             ...
                             truth.
    - `equipment`         -- a tufting-machine/weaving-loom/backing-
    ...
    - `maintenance`       -- a scheduled tufting-machine, weaving-loom,
                             or backing-line maintenance-window DRAFT
                             against a piece of equipment
                             (`carpetops.registry`'s
                             `register-maintenance`). Dedicated
                             `:scheduled?` double-schedule guard (never
                             a `:status` value).
    - `shipments`         -- a proposed outbound carpet/rug shipment
                             DRAFT (`carpetops.registry`'s
                             `register-shipment`).
  ...
  audit trail a plant owner or carpet/rug buyer trusting this
  coordinator needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (carpetops.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (carpetops.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :quality-grade :first-quality :product "cut-pile-tufted-carpet"
                :area-square-meters 20000.0 :pile-density-tufts-per-sqm 85000.0
                :tuft-bind-strength-lbf 4.5 :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-area-square-meters 5000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :quality-grade :second-quality :product "loop-pile-woven-rug"
                :area-square-meters 6000.0 :pile-density-tufts-per-sqm 60000.0
                :tuft-bind-strength-lbf 3.0 :defect-rate-percent 3.0
                :verified? true :registered? true
                :shipped-area-square-meters 5700.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :quality-grade :substandard :product "berber-loop-carpet-tile"
                :area-square-meters 15000.0 :pile-density-tufts-per-sqm 12000.0
                :tuft-bind-strength-lbf 0.8 :defect-rate-percent 18.0
                :verified? false :registered? false
                :shipped-area-square-meters 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :tufting-machine
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :weaving-loom
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `carpetops.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `carpetops.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered tufting-machine
  unit (schedulable for maintenance), one UNVERIFIED/unregistered
  weaving-loom unit (blocks any maintenance scheduling against it) --
