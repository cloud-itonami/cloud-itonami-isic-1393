(ns carpetops.advisor
  "CarpetAdvisor -- the *contained intelligence node* for the
  carpet/rug-plant plant-operations coordination actor.

  It normalizes production-batch patches (quality-grade/area/pile-
  density/tuft-bind-strength/defect-rate), drafts a tufting-machine,
  weaving-loom, or backing-line maintenance scheduling proposal
  against a piece of equipment, drafts an equipment-safety/quality-
  defect concern flag, and drafts an outbound-carpet-shipment
  ...
  a real tufting/weaving/backing-line-equipment actuation or freight
  dispatch. Every output is censored downstream by
  `carpetops.governor` before anything touches the SSoT -- see README
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct tufting/
                                 ; weaving/backing-line-equipment-
                                 ; control effect
  ...
  `carpetops.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [carpetops.registry :as registry]
...
(defn- log-production-batch
  ...
  area, pile-density, tuft-bind-strength or defect-rate. High
...
(defn- schedule-maintenance
  "Draft a tufting-machine, weaving-loom, or backing-line maintenance-
  ...
  rationale, but `carpetops.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  ...
  `carpetops.phase`: no phase ever adds this op to a phase's `:auto`
  set; `carpetops.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound carpet/rug shipment coordination proposal against
  a production batch. The advisor passes through the caller's own
  claimed area -- it does NOT invent one, and `carpetops.governor`
...
(def ^:private system-prompt
  (str "あなたはタフテッド・織絨毯工場プラント運用コーディネーターの助言者です。"
       ...
       "タフティング機・織機・裏張りラインの設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `carpetops.governor`
