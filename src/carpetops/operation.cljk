(ns carpetops.operation
  "CarpetOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (CarpetAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Carpet & Rug Plant
  ...
  (:require [langgraph.graph :as g]
            ...
            [carpetops.advisor :as advisor]
            [carpetops.governor :as governor]
            [carpetops.phase :as phase]
...
(defn build
  "Compiles a CarpetOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `carpetops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CarpetAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Carpet & Rug Plant Operations Governor -- independent censor.
      (g/add-node :govern
