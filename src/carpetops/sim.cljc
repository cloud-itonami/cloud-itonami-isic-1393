(ns carpetops.sim
  ...
  tufting machine, weaving loom, or backing line (permanently blocked,
  ...
  patch with an implausible pile-density reading, a production-batch
  patch with an implausible tuft-bind-strength reading, and a
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, tufting-machine -- escalates, approve) ==")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:quality-grade :first-quality}}
                       coordinator))

    (println "== unrecognized op -> HARD hold ==")
    (println (exec-op actor "t6"
                       {:op :actuate-tufting-machine :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered weaving loom -> HARD hold) ==")
    ...
    (println "== log-production-batch batch-001 with an implausible pile-density reading -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:pile-density-tufts-per-sqm 9999999.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible tuft-bind-strength reading -> HARD hold ==")
    (println (exec-op actor "t14"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:tuft-bind-strength-lbf 999999.0}}
                       coordinator))
