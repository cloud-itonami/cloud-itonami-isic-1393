(ns carpetops.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `carpetops.store` ns docstring for why a second (Datomic-backed)
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:quality-grade :first-quality}})
    (is (= :first-quality (:quality-grade (store/batch s "batch-001"))))
